...
def main():
    ...
    muni_states = municode_nav.scrape_states()
    state = input("State: ").lower()
    municode_nav.go(muni_states[state])
    muni_cities = municode_nav.scrape_cities()
    for city in muni_cities:
        print(city)
    muni = input("Municipality: ").lower()
    municode_nav.go(muni_cities[muni])
    #municode_nav.go("https://library.municode.com/ca/yreka/codes/code_of_ordinances")
    muni_titles = municode_nav.scrape_titles()
    titles_list = ""
    for title in muni_titles:
        titles_list += title + ", "
    titles_list = titles_list[:-2]
    random_titles = random.sample(list(muni_titles.keys()), 3)
