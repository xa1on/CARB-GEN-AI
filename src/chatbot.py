...
LOG_PROMPTS = True # logs prompts generated by the chatbot
...
CONFIGS = {
    "thinking": types.GenerateContentConfig(
        system_instruction=[
            ...
            """Quotations must be exact content from inside the provided link with no modification. Quotations must contain the exact wording of the document.
...
"""Make sure to check your work whenever asked a question"""
        ],
    ...
    for attempt in response_json:
        ...
        if current_name in muni_names:
            ...
            if not depth or muni_nav.contains_child(): # if theres child entries, we'll use those instead to get our answer
                if not depth:
                    depth += 1
                prompt, response = answer(muni_nav, client, muni, muni_names[current_name], query, depth + 1, definitions_link)
            else:
                log("## ANSWERING\n\n")
